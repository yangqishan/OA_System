...
<script>
		...
		$('.tablefirst').on('click',function(){
			 if(${page.first?string('true','false')}==false){
				 $('.thistable').load('${url}?page=0${(sort)!''}&search=${(search)!''}');
			 }
		});
		$('.tableup').on('click',function(){
			 if(${page.first?string('true','false')}==false){
				 $('.thistable').load('${url}?page=${(page.number)-1}${(sort)!''}&search=${(search)!''}');
			 }
		});
		...
		$('.tablelast').on('click',function(){
			if(${page.last?string('true','false')}==false){
				$('.thistable').load('${url}?page=${(page.totalPages)-1}${(sort)!''}&search=${(search)!''}');
			 }
