...
<div class="row" style="padding-top: 10px;">
	...
	<div class="col-md-10 text-right">
		<ahref="/index" target="_parent"><span class="glyphicon glyphicon-home"></span> 首页</a> > <a
		...
		<div class="bgc-w box">
			...
			<form action="useredit" method="post" onsubmit="return check();">
				<div class="box-header">
					<h3 class="box-title">
						<a href="usermanage" class="label label-default" style="padding: 5px;">
					...
					<div class="box-body">
						...
						<div class="row">
							...
							<div class="col-md-6 form-group">
								...
								<#if  user?? && positions1.name==user.position.name>
								<select id="dep" class="deptselect form-control" name="deptid">
								...
								<select class="deptselect form-control" name="deptid">
									...
									<#list depts as dept>
										<#if user??>
											<#if user.dept.deptName!=dept.deptName>
							...
							<div class="col-md-6 form-group">
								...
								<#if user?? && positions1.name==user.position.name>
								<select id="pos" class="positionselect form-control" name="positionid">
							...
							<div class="col-md-6 form-group">
								<label class="control-label"> <span>皮肤</span>
									...
									<option value="red">朱丹红</option>
									<option value="yellow">大地黄</option>
								</select>
							</div>
				...
				<div class="box-footer">
					...
					<input class="btn btn-default" id="cancel" type="submit" value="取消" href="usermanage" />
				</div>
			</form>
		</div>
	</div>
...
<script type="text/javascript">
    $(function(){
        $("#dep").click(function(){
            alert("此人职位是部门经理，无法在此更改部门，请到部门管理处进行人事调动操作！");
            return false;
        });
    });
    $(function(){
        $("#pos").click(function(){
            alert("此人职位是部门经理，无法在此更改职位，请到部门管理处进行人事调动操作！");
            return false;
        });
    });
...
function check() {
	...
	$('.form-control').each(function(index) {
		...
		if ($(this).val() == "") {
			// 排除哪些字段是可以为空的，在这里排除
			if (index == 9 || index == 10 || index == 11 || index == 12 || index == 15) {
				return true;
			}
...
function isCardNo(card) { 
 var pattern = /(^(\d{6})(19|20)(\d{2})(1[0-2]|0[1-9])(0[1-9]|[1-2][0-9]|3[0-1])(\d{3})(\d|X|x)$)/;
