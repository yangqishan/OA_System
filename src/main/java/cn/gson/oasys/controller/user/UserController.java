...
import cn.gson.oasys.controller.mail.MailUtil;
...
public class UserController {
	...
	public String userPaging(Model model,@RequestParam(value="page",defaultValue="0") int page,
		...
		if(StringUtil.isEmpty(search)){
			...
			userspage = udao.findnamelike(search, pa);
		}
		List<User> users=userspage.getContent();
		model.addAttribute("users",users);
		model.addAttribute("page", userspage);
	...
	public String usereditget(@RequestParam(value = "userid",required=false) Long userid,Model model) throws Exception {
		Long deptid = 1L;
		if(userid!=null){
		...
		List<Position> positions = (List<Position>) pdao.findByDeptidAndNameNotLike(deptid, "%经理");
		Position positions1 =pdao.findOneByDeptidAndNameLike(deptid,"%经理");
		...
		model.addAttribute("deptid",deptid);
		model.addAttribute("positions1",positions1);
...
//		System.out.println(user.getHireTime().getTime());
		Date time = new Date();
		...
		if(user.getUserId()==null){
			...
			user.setDept(dept);
			user.setRole(role);
			user.setHireTime(time);
			user.setPosition(position);
			user.setFatherId(dept.getDeptmanager());
			udao.save(user);
			MailUtil.send_mail(user.getEamil(), "尊敬的"+user.getRealName()+"先生，你好:<br>" +
					"恭喜你通过了我们的面试，成功进入我们公司，现将你在公司的详细信息公布如下:<br>" +
					"部门:"+dept.getDeptName()+",     职位:"+position.getName()+",<br>" +
					"入职时间:"+timeformat+",     薪酬:"+user.getSalary()+",<br>" +
					"登录用户名:"+user.getUserName()+",      登录密码:123456,<br>" +
					"因初始密码为默认密码，请尽快登录系统修改自己的密码！<br>" +
					"i726欢迎你的加入！");
		}else{
