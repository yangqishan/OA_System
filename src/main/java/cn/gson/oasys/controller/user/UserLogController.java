...
public class UserLogController {
	...
	//用来查找用户记录
			...
			public String test3df342(@RequestParam(value="page",defaultValue = "0")int page,
				...
				getuserlogrecord(page, session, model, search, time, icon);
			...
			//用来查找用户记录
				...
				public void getuserlogrecord(int page, HttpSession session, Model model, String search, String time,
						String icon) {
					long userid=Long.valueOf(session.getAttribute("userId")+"");
					setTwo(model, search, time,icon);
					Page<LoginRecord> page4=userLogRecordService.ulogpaging(page, search, userid, time);
	...
	//用来查找用户记录
		...
		public String test3df(@RequestParam(value="page",defaultValue = "0")int page,
			...
			System.out.println("11"+search);
			getuserlog(page, session, model, search, time, icon,request);
		...
		//用来查找用户记录
			...
			public String test3dfrt(@RequestParam(value="page",defaultValue = "0")int page,
					...
					@RequestParam(value="search",required=false)String search,
					@RequestParam(value="time",required=false)String time,
					@RequestParam(value="icon",required=false)String icon, HttpServletRequest request) {
				getuserlog(page, session, model, search, time, icon,request);
			...
			public void getuserlog(int page, HttpSession session, Model model, String search, String time,
								   String icon, HttpServletRequest request) {
				long userid=Long.valueOf(session.getAttribute("userId")+"");
				setTwo(model, search, time,icon);
				Page<UserLog> page3=userLogService.ulogpaging(page, search, userid, time);
			...
			//记忆两种规则
			private void setTwo(Model model, String search, Object time,Object icon) {
				...
				if(!StringUtils.isEmpty(search)){
					model.addAttribute("basekey", search);
					model.addAttribute("sort", "&basekey="+search);
				}
			}
