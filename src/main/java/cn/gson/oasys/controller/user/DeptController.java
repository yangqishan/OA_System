...
public class DeptController {
	...
	public String deptmanage(Model model,@RequestParam(value="page",defaultValue="0") int page,
							 @RequestParam(value="size",defaultValue="5") int size) {
		List<Dept> depts = (List<Dept>) deptdao.findAll();
		//System.out.println(depts);
//		Pageable pa=new PageRequest(page, size);
//		Page<Dept> deptspage = deptdao.findAll(pa);
//		List<Dept> depts = deptspage.getContent();
		model.addAttribute("depts",depts);
//		model.addAttribute("page",deptspage);
//		model.addAttribute("url","deptmanage");
	...
	public String readdept(@RequestParam(value = "deptid") Long deptId,Model model){
		...
		List<User> deptusers = udao.findByIsLockAndDept(0,dept);
