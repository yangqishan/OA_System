...
public class AttendceController {
	...
	public String Datag(HttpSession session, Model model, HttpServletRequest request) throws InterruptedException, UnknownHostException {
		...
		String hostName=ia.getHostName().toString();
		// 时间规范
		String start = "08:00:00", end = "18:00:00";
		...
		Long useid = attenceDao.finduid(nowdate,hostName);
		System.out.println(useid);
		System.out.println(userId);

		// 查找用户当天的所有记录
		Integer count = attenceDao.countrecord(nowdate, userId);
		if (useid!=null&&!useid.equals(userId)){
			System.out.println("----不能代签");
			model.addAttribute("error","0");
		}
		else if (count == 1 && (hourminsec.compareTo(end))<0){
            System.out.println("你已经签到过上班了，请等下班后再签到吧,下班签到时间为18:00到18:30");
            model.addAttribute("error","3");

        }
		else {
			if (hourminsec.compareTo("18:30:00") > 0) {
				// 在18:30之后签到无效
			...
			} else if (hourminsec.compareTo("18:30:00") > 0) {
				if (count == 0) {
					typeId = 4;
					statusId = 13;
					attends = new Attends(typeId, statusId, date, hourmin, weekofday, attendip, hostName, user);
					attenceDao.save(attends);
				}

			} else if ((hourminsec.compareTo("07:00:00") > 0) && (hourminsec.compareTo("18:30:00") < 0)) {
	...
	//修改考勤时间
//	@RequestMapping(value = "attendcetimesedit",method = RequestMethod.POST)
//	public String DSAGen(Model model,HttpSession session,HttpServletRequest request) {
//
//		return "attendce/attendcetimeedit";
//	}
...
//	}
	...
	public String test4(Model model, HttpSession session, HttpServletRequest request) {
		...
		SystemStatusList statusList=  statusDao.findByStatusModelAndStatusName("考勤管理", statusname);
	...
	private void typestatus(HttpServletRequest request) {
		List<SystemTypeList> type = (List<SystemTypeList>) typeDao.findByTypeModel("考勤管理");
		List<SystemStatusList> status = (List<SystemStatusList>) statusDao.findByStatusModel("考勤管理");
