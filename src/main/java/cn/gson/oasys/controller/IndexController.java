...
//				simpleDateFormat.parse(scheduleList.getStartTime()).getTime();  
	...
	public String test2(HttpSession session, Model model, HttpServletRequest request) {
		...
		List<SystemTypeList> ptype = (List<SystemTypeList>) typeDao.findByTypeModel("工作计划");
		List<SystemStatusList> pstatus = (List<SystemStatusList>) statusDao.findByStatusModel("工作计划");
		...
		List<SystemStatusList> processstatus = (List<SystemStatusList>) statusDao.findByStatusModel("流程管理");
