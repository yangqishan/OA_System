...
public interface AttendceDao  extends JpaRepository<Attends, Long>{
	...
	//
	@Query(nativeQuery = true,value = "select attends_user_id from aoa_attends_list a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_hostname=?2 ")
    Long finduid(String date,String hostname);
